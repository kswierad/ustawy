...
public class ConsContent extends AbstractContent{
    ...
    public void makeID(String madeFrom){
        switch (this.type) {
            case Section: this.id=madeFrom.substring(10);
                break;

            case Article: this.id=madeFrom.substring(6);
                break;
            case Paragraph:
            case Point: this.id=madeFrom.substring(1,3);
                if(this.id.substring(1).equals(".")||this.id.substring(1).equals(")")) this.id=this.id.substring(0,1);
